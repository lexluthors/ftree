...
- ⌨️ **丰富键盘操作** - vim 风格快捷键
- 📋 **多选复制** - 选中多个文件后一次性复制路径
- 📎 **复制文件** - 右键复制文件/目录，可直接粘贴到文件管理器和微信、QQ
...
| 右键点击行 | 打开操作菜单：复制文件 / 粘贴文件（目录） / 运行脚本 / 删除 |
...
- **右键 → `1. 复制文件`**：把选中的（或光标所在的）文件/目录以「文件」形式放进系统剪贴板
- **右键 → `2. 粘贴文件`**（光标在目录上时）：把剪贴板里的文件复制进该目录，同名自动加 `(1)` 后缀
- 复制后可粘贴到：
  ...
  | Dolphin、Kate、浏览器等 Qt/KDE 程序 | ✅ |
  | macOS：Finder、微信 mac | ✅ |

- 实现要点：X11 剪贴板按 `TARGETS` 协商格式，而 `xclip` 一次只能持有一种格式。
  ftree 因此自带一个剪贴板守护进程（`ftree --clip-daemon`），同时提供
  `text/uri-list`（聊天软件/KDE 认这个）和 `x-special/gnome-copied-files`（GNOME 文件管理器认这个），
  且 URI 做了百分号编码，空格 / 中文 / `#` / `&` 文件名都能正确传递。
  守护进程在 ftree 退出后仍持有剪贴板，直到被其他程序覆盖。
...
- **剪贴板**：X11 用自带守护进程（`ftree --clip-daemon`，基于 x11rb，一次提供多种格式），失败时回退 xclip；Wayland 用 wl-copy；macOS 用 pbcopy / osascript
- **性能**：懒加载目录，支持数千个文件的目录
